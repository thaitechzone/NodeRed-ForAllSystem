...
> **เป้าหมาย**: ย้าย Docker Stack (Node-RED + N8N + MQTT + InfluxDB + Grafana + Ngrok)  
...
nodered/settings.js
grafana/provisioning/datasources/influxdb.yml ← auto-config Grafana datasource
```
...
| **InfluxDB** | `http://YOUR_VPS_IP:8086` | INFLUXDB_USERNAME/PASSWORD จาก `.env` |
| **Grafana** | `http://YOUR_VPS_IP:3000` | GRAFANA_ADMIN_USER/PASSWORD จาก `.env` |
...
| เข้าถึง InfluxDB | `localhost:8086` | `VPS_IP:8086` |
| เข้าถึง Grafana | `localhost:3000` | `VPS_IP:3000` |
...
| InfluxDB ไม่ start | password สั้นเกิน | `INFLUXDB_PASSWORD` ต้องยาว 8+ ตัว |
| Grafana datasource error | Token ไม่ตรง | ตรวจ `INFLUXDB_TOKEN` ใน `.env` |
| ไม่เห็นข้อมูลใน Grafana | Node-RED ไม่ส่ง | ตรวจ InfluxDB Out node ใน Node-RED |
...
- Grafana จะเข้าถึงได้ที่ `https://grafana.yourdomain.com` แบบ secure
